fisherTables <- function(x,y,Data,reverse=FALSE,sortTable=ifelse(nrow(tab)>2,TRUE,FALSE))
  {
    ...
    R[[3]] <- htmlCross(tab,sortTable=sortTable)
    R[[4]] <- htmlCross(100*prop.table(tab,1),margins=FALSE,digits=1,sortTable=sortTable)
    R[[6]] <- R[[8]] <- htmlText("")
...
##      {
##        warning(paste("Cannot execute Fisher's Exact Test for:\n",x,"by",y,"(table too large)\n"))
##        R[[7]] <- htmlText("")
##        R[[5]] <- htmlText("Cannot test for association; table too big")
##        return(htmlList(R))
##      }
